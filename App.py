...
def main(): 
    ...
    if app_mode == "App Source Code":
        ...
        st.image(image, use_column_width=True)
        st.markdown("<h4 style='text-align: right;'>השתמשנו בדאטה שנמצא באתר בית המשפט העליון בארצות הברית מכיון שאילו נתונים פתוחים ומותרים לשימוש. הדאטה מכיל רשומות של משפטים שהתקיימו ותיוג כיוון המשפט - ליברלי מול שמרני </h4>", unsafe_allow_html=True)
        st.markdown("<h4 style='text-align: right;'>תחילה, עברנו על הדאטה וסיננו את הפיצ'רים היכולים להשפיע על הטיה כדוגמת שם השופט, כתובות וכו'.כך שכל הפ'יצרים שנשארו אינם מצביעים על אוכלוסיה מסוימת ולכן לא יכולה להיות הטיה במודל. לאחר מכן, בחרנו פיצ'רים רלוונטים באמצעות שיטת פיצ'ר סלקשן</h4>", unsafe_allow_html=True)
        ...
        if col1.button('הטיה'):
              ...
              st.markdown("<h4 style='text-align: right;'>אפליה בהעסקה – חברות שונות משתמשות במערכות המבוססות על בינה מלאכותית כדי לבצע הליכי מיון לעובדים, ניכר כי השימוש בבינה מלאכותית בתחומים אלה מובילה להפליה על רקע גזע ומין. לעיתים ההפליה אינה נוצרת במכוון אלא משקפת את האפליה הקיימת במציאות(4)</h4>", unsafe_allow_html=True)
              st.markdown("<h4 style='text-align: right;'>אפליה בשירות – בנקים וגופים מלווים, משתמשים לעיתים במערכות בינה מלאכותית כדי לנבא את ההחזר הפוטנציאלי של לווים(5),  גם כאן ניתן לראות כי לעיתים מרחשת אפליה בין גברים ונשים(6)</h4>", unsafe_allow_html=True)
              ...
              st.markdown("<h4 style='text-align: right;'>כדי למנוע סיטואציה בה המערכת שלנו, המבוססת כאמור על בינה מלאכותית, תפגע בזכות לשוויון של כל אדם, המעוגנת בחוקה של ארצות הברית ובחוק יסוד: כבוד האדם וחירותו, ישנם משתנים שהושמטו באופן מכוון מהמערכת. שכן הנתונים על בסיסם ביצעה המערכת שלנו את הליך הלמידה כוללת משתנים שונים, היוצרים יחד תמונה כללית לגבי פסיקתו של בית המשפט בארצות הברית בהתאם לכל מקרה – ליברלי או שמרני. לאחר בדיקות רבות הצלחנו לבודד משתנים מהותיים כמו: מין של התובע, הנתבע והשופטים, שמות הצדדים, גזע וכתובות המגורים אשר עשויים להעיד על מצב סוציואקונומי. זאת ללא פגיעה באיכות ודיוק התוצאה המתקבלת</h4>", unsafe_allow_html=True)
              st.write("--------------------------------------------------------------")
        ...
        st.markdown("<h4 style='text-align: right;'>פרויקט זה בוצע במסגרת הקורס בינה מלאכותית ומוסר. רצינו לנצל את העובדה שהצוות מורכב ממומחי תוכן בעולם הבינה המלאכותית וכן מעולם המשפטים. לצורך כך לקחנו בעיה של חיזוי כיוון בית המשפט העליון בארה'ב ובנינו מודל בינה מלאכותית שיחזה זאת. האתר הזה מסביר את העבודה שנעשתה ומנגיש את המודל לשימוש</h4>", unsafe_allow_html=True)
