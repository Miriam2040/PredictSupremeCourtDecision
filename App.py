...
def main(): 
    ...
    if app_mode == "App Source Code":
        ...
        st.markdown("<h4 style='text-align: right;'>בית המשפט העליון בארצות הברית מורכב מתשעה שופטים, כאשר המטרה של המערכת שלנו היא להבין כיצד הם יפסקו בכל מקרה ומקרה, האם הפסיקה תהיה שמרנית? פסיקה בהתאם לחוק או שמא ליברלית? על ידי אקטיביזם שיפוטי, כאשר בית המשפט רואה עצמו כמגן על זכויות אדם(1)</h2>", unsafe_allow_html=True)
