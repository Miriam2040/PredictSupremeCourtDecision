...
def main(): 
    ...
    if app_mode == "App Source Code":
        ...
        if col1.button('הטיה'):
              ...
              st.markdown("<h4 style='text-align: right;'>(3)אפליה בהחלטות שיפוטיות – מערכת המשפט בארצות הברית משתמשת במערכות המבוססות על בינה מלאכותית. אותן מערכות מדרגות את רמת הסיכון העתידית של פושע, זאת כדי לסייע לשופט לקבל החלטה בנוגע לעונש הראוי. מחקרים בתחום מראים כיצד מערכות אלה מפלות על בסיס גזע</h4>", unsafe_allow_html=True)
              st.markdown("<h4 style='text-align: right;'>(4)אפליה בהעסקה – חברות שונות משתמשות במערכות המבוססות על בינה מלאכותית כדי לבצע הליכי מיוון לעובדים, ניכר כי השימוש בבינה מלאכותית בתחומים אלה מובילה להפליה על רקע גזע ומין. לעיתים ההפליה אינה נוצרת במכוון אלא משקפת את האפליה הקיימת במציאות</h4>", unsafe_allow_html=True)
              st.markdown("<h4 style='text-align: right;'>(6)אפליה בשירות – בנקים וגופים מלווים, משתמשים לעיתים במערכות בינה מלאכותית כדי לנבע את ההחזר הפוטנציאלי של לווים(5),  גם כאן ניתן לראות כי לעיתים מרחשת אפליה בין גברים ונשים</h4>", unsafe_allow_html=True)
