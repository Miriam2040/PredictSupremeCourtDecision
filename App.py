...
def main(): 
    ...
    if app_mode == "App Source Code":
        ...
        st.markdown("<h4 style='text-align: right;'>בתחילה, עברנו על הדאטה וסיננו את הפיצ'רים היכולים להשפיע על הטיה כדוגמת שם השופט, כתובות וכו'.כך שכל הפ'יצרים שנשארנו אינם מצביעים על אוכלוסיה מסוימת וךכן לא יכולה להיות הטיה במודל. לאחר מכן, בחרנו פיצ'רים רלוונטים באמצעות שיטת פיצ'ר סלקשן</h4>", unsafe_allow_html=True)
