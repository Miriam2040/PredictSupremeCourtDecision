...
def main(): 
    ...
    if app_mode == "App Source Code":
        ...
        if col1.button('הטיה'):
              ...
              st.markdown("<h4 style='text-align: right;'>אפליה בשירות – בנקים וגופים מלווים, משתמשים לעיתים במערכות בינה מלאכותית כדי לנבע את ההחזר הפוטנציאלי של לווים(5),  גם כאן ניתן לראות כי לעיתים מרחשת אפליה בין גברים ונשים(6)(7)</h4>", unsafe_allow_html=True)
              st.markdown("<h4 style='text-align: right;'>אפליה במחירים – ישנם מוצרים כמו טיסות וביטוחים אשר עשויים להיות מתומחרים באופן שונה עקב פרמטרים מסוימים (רכישות קודמות, מיקום המשתמש, המכשיר ממנו מתבצעת הקניה וכו') שהלוקח אינו מודע לקיומם. כך נוצרת הטיה על בסיס מעמד כלכלי(8)</h4>", unsafe_allow_html=True)
