...
def main(): 
    ...
    if app_mode == "App Source Code":
        ...
        st.markdown("<h4 style='text-align: right;'>  אחרי שאימנו את המודל של סט האימון בדקנו את ביצועיו באמצעות סט הטסט. עבור סט הטסט קיבלנו דיוק של 78% שזה משמעותית יותר טוב מניחוש רגיל של 50% דיוק. כמו כן, בדקנו מהי חשיבות הפיצ'רים בהשפעה על תוצאות המודל: </h4>", unsafe_allow_html=True)
